...
import { Modal,  Card, Col, Row, Pagination, Select } from 'antd';
import { CheckOutlined, CloseOutlined } from '@ant-design/icons';
import { setFilters, filterData, sortingAsc, sortingDesc } from "./filterFunction"

class AvtoPregled extends Component {
    constructor(props) {
        super(props);
        this.state = {
            open: false,
    ...
    render() {
        const { open, avti, selectedModeli, znamke, currentPage, cardsPerPage, goriva, menjalniki, record } = this.state
        const cene = [
            ...
            <div>
                <div style={{ display: 'flex', flexWrap: 'wrap' }}>
                    ...
                    <Select allowClear showSearch placeholder={"Sortiranje"}
                        style={{ width: 205, marginRight: 5, marginBottom: 5,  marginLeft: 'auto'}} options={sort}
                        filterOption={(input, option) => (option?.label ?? '').toLowerCase().includes(input.toLowerCase())}
...
const Avti = () => {
    ...
    return <AvtoPregled navigate={navigate} />;
};
